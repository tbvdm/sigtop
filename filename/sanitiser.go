...
func ParseOS(s string) (OS, error) {
	...
	case "windows":
		return Windows, nil
	default:
		return Native, fmt.Errorf("invalid OS: %s", s)
	}
}

type Sanitiser struct {
	sanitise func(string) string
}

func NewSanitiser(os OS) *Sanitiser {
	if os == Native {
		switch runtime.GOOS {
		case "darwin":
			os = Macos
	...
	sanitise := sanitiseUnix
	switch os {
	case Macos:
		sanitise = sanitiseMacos
	case Windows:
		sanitise = sanitiseWindows
	}

	return &Sanitiser{sanitise: sanitise}
}

func (s *Sanitiser) Sanitise(name string) string {
	return s.sanitise(name)
}

func sanitiseMacos(name string) string {
	if name == "" || name == "." || name == ".." {
		return name + "_"
	}
...
func sanitiseUnix(name string) string {
	if name == "" || name == "." || name == ".." {
		return name + "_"
	}
...
func sanitiseWindows(name string) string {
	// Sanitation based on
	...
	// Handle filenames ending with a space or a dot (which also takes care
	// of "." and "..")
